...
import { ArrowLeft, Eye, Edit3, Lock, Globe, Save } from "lucide-react";
import Link from "next/link";
import { cn } from "@/lib/utils";
import { upsertPost } from "@/app/actions/post";
...
interface NewEditorClientProps {
  userId: string;
}

export function NewEditorClient({ userId }: NewEditorClientProps) {
  ...
  const onSubmit = async (data: PostValues) => {
    setIsSaving(true);
    const result = await upsertPost(null, {
      title: data.title,
      slug: data.slug,
